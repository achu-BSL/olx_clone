    ...
    <div>
      <div className="flex justify-center flex-col w-full px-6 md:px-16 py:3 md:py-8 w-max-[1228px]">
        <div className="relative h-80 w-full bg-black">
          <ChevronLeft
            className="text-white absolute top-[50%]"
            onClick={() => changeSelectedImgIdx(-1)}
            fontSize="large"
          />
          <img
          ...
          />
          <ChevronRight
            className="text-white absolute right-0 top-[50%]"
            onClick={() => changeSelectedImgIdx(1)}
            fontSize="large"
          />
        </div>
          ...
          <div className="px-16 py-12 flex-1 shadow-md">
            ...
            <p>
              ...
              <span className="font-semibold">
                {currProductDetails?.owner.username}
              </span>
            </p>
            <p>
              Email:{" "}
              <span className="font-semibold">
                {currProductDetails?.owner.useremail}
              </span>
            </p>
          </div>
          <div className="px-8 py-12 flex flex-col gap-2 shadow-lg border border-slate-600 border-opacity-30 rounded-md">
            <h2 className="font-semibold text-4xl">
            &#8377; {IndFormat.format(+currProductDetails!.productPrice)}
            </h2>
